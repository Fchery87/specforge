...
Analyze the provided project brief and extract or infer the following constitutional elements:
...
- Accessibility level (WCAG 2.1 AA/AAA compliance targets)
- Performance budgets (bundle size limits, TTFB targets, LCP thresholds)
- Security requirements (authentication patterns, authorization models, data protection)
 ...
 */
export function injectConstitutionContext(basePrompt: string, constitution: string): string {
  return `${basePrompt}
